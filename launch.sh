...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "myserviceapp")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name myserviceapp apps/myserviceapp:1.0
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app myserviceapp:1.0
    oc expose svc/myserviceapp
else

	echo "Launching the application locally..."
	pattern="myserviceapp"
	files=( $pattern )
	cd ${files[0]}
